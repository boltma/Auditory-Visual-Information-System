...
def test_task2(root_path):
    ...
    audio_loader = torch.utils.data.DataLoader(audio_ds, 4, False, num_workers=0)
    ...
    video_loader = torch.utils.data.DataLoader(video_ds, 4, False, num_workers=0)
    ...
    for i in range(10):
        ...
        matching_ds = matching_dataset(mode="test", transforms=transform, test_path=root_path, test_audio=audio_i,
                                       test_video=video_i)
        matching_loader = torch.utils.data.DataLoader(matching_ds, 4, False, num_workers=0)
